  ...
  async function performImageAnalysis(analysisId, dietImage, userId, dietType, dietDate) {
    try {
      ...
      for (const food of analysisResult.음식상세) {
        ...
        // menuList에 음식명이 없는 경우, 새로운 메뉴 아이템 생성
        if (!menuItem) {
          menuItem = await MenuList.create({
            menuName: food.음식명,
            calorie: food.영양정보.칼로리,
            // carbohydrate: food.영양정보.탄수화물,
            // protein: food.영양정보.단백질,
            // fat: food.영양정보.지방
          });
        }
  
        await DietLogDetail.create({
          dietLogId: newDietLog.dietLogId,
          menuId: menuItem.menuId,
          quantity: food.예상양 / 100 // 예상양이 g 단위이므로 100으로 나눔
        });
      }
